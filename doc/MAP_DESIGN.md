...
| 5 | `Ice` (빙하) | ❌ | ❌ | 남극 등(위도 -60° 이하 육지). 흰색, 통행 불가 |

> 최소 MVP는 `DeepSea / Land / Port` 3종으로도 시작 가능. 현재 변환은 DeepSea/ShallowSea/Land/Mountain/Ice 사용(Port는 도시 단계).
...
대형 월드(현재 **4096×2048 = 8,388,608셀**)도 가볍게 다루기 위해 **1D 평면 배열 + 레이어 분리**를 쓴다.
...
└─ build_world_data.py            ← GeoJSON+고도 → 4096x2048 PNG 2장(지형/고도)
...
├─ Authoring/world_terrain_4096x2048.png ← 지형 분류색 (임포터 입력)
├─ Authoring/world_height_4096x2048.png  ← 고도 그레이스케일 (임포터 입력)
...
2. ✅ 실제 지구 데이터 → 4096×2048 PNG 변환 + 임포터 (**로직용** WorldMapData)
3. ✅ 비주얼 = **실사 지구 이미지(EARTH.jpg) 평면** + 한국 중심 입체 부감 카메라 + 줌

> 비주얼 방식 변천: ① 분류색 래스터(도트) → ② 벡터 해안선 메시(상충·복잡) → ③ **실사 이미지 평면**(HorizontalChild 참조, 채택). ①②는 제거됨.
> 한계: EARTH.jpg 2048px라 국가 단위 줌에선 흐릿. 세계~지역 뷰에 최적. 더 또렷한 근접 줌이 필요하면 고해상 이미지/벡터 병행 재검토.

다음 후보:
4. 배 오브젝트 이동 + `IsNavigable` 통행 판정 (= 첫 "동작하는 맵" MVP)
5. 항구/도시 진입, A* 자동 항해
6. 이후 의뢰·무역·전투·명성·동료 시스템 (각각 맵을 *참조*)

### 확정된 결정
- 지형 타입: DeepSea/ShallowSea/Land/Mountain/Ice (Port는 도시 단계에서). 위도 -60° 이하 육지=Ice(흰색).
- 격자 4096×2048, cellSize 1, XZ 평면 매핑(x→X, y→Z), (0,0)=남서
